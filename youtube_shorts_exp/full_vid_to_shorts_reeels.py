...
def convert_to_vertical_webm(input_path, output_path, script_dir):
    ...
    # Determine a focused crop area to better center HUD/kills for vertical output.
    src_w, src_h = get_video_dimensions(input_path)
    focus_x_ratio = 0.75  # center the crop around 75% width (right side HUD)
    if src_w and src_h:
        # Compute a maximal 9:16 crop that fits inside source (avoid pad smaller-than-input errors)
        target_aspect = 9.0 / 16.0
        src_aspect = src_w / src_h
        if src_aspect >= target_aspect:
            # source is wider: use full height and crop width
            crop_h = src_h
            crop_w = int(src_h * target_aspect)
        else:
            # source is taller: use full width and crop height
            crop_w = src_w
        ...
        # crop to 9:16 area then scale exactly to TARGET (no pad)
        video = (
            ffmpeg
            .input(input_path)
            .filter('crop', crop_w, crop_h, x1, y1)
        ...
        video = (
            ffmpeg
            .input(input_path)
            .filter('scale', TARGET_WIDTH, -2, flags='lanczos')
...
def convert_webm_to_mp4(input_folder, output_folder, label):
    ...
    for file in os.listdir(input_folder):
        if file.lower().endswith('.webm'):
            ...
            # prefer numeric crop centered at focus_x_ratio if available
            src_w, src_h = get_video_dimensions(input_path)
            focus_x_ratio = 0.75
            if src_w and src_h:
                target_aspect = 9.0 / 16.0
                ...
                x1 = max(0, min(src_w - crop_w, center_x - crop_w // 2))
                y1 = max(0, (src_h - crop_h) // 2)
                crop_filter = f"crop={crop_w}:{crop_h}:{x1}:{y1},scale=1080:1920:flags=lanczos"
            else:
                crop_filter = "crop=in_h*9/16:in_h:(in_w-out_w)/2:0,scale=1080:-2"
            subprocess.run([
